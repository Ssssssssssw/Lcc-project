...
function adtChd(reqData) {
    ...
    //  单程往返按钮的类名会不定时的改变 需要定时维护
    document.querySelector(".list-button:nth-of-type(1)").click()
    //  成人儿童数的外层div类名不定时改变
    // document.querySelector(".ev_ew,.ex_ey").click()
...
function fromData(reqData) {
    ...
    setTimeout(function () {
        // let itemAll = document.querySelectorAll(".b3o_b3p")
        ...
        // let itemAll = document.querySelectorAll(".cjr_cjs")
        // let arr = [].slice.apply(itemAll)
        // arr.pop().childNodes.forEach(item => {
        //     item.childNodes.forEach((it, id) => {
        //         it.itemArr = [].slice.apply(it.childNodes)
        //         it.itemArr.splice(0, 1)
        //         it.itemArr.forEach(j => {
        //             j.arr = j.innerText.split("\n")
...
function dateFun(reqData) {
    ...
    setTimeout(function () {
        ...
        //  页面月份标签 ；类名会不定时的改变
        // let monthDom = document.querySelectorAll("#calendar-tabs-D63EA272-DACD-47E4-9D91-C874C3C1E7BC-panel > div > div > div.b63_b64.b65_b66 > div > div")
        ...
        if (!monthStatus) {
            for (let i = 0; i < arr[0].childNodes.length; i++) {
                ...
                if (arr[0].childNodes[i].arr[0] != year || arr[0].childNodes[i].arr[1] != month) {
                    // 下一页的月份按钮类名不定时改变
                    // document.querySelector("#calendar-tabs-D63EA272-DACD-47E4-9D91-C874C3C1E7BC-panel > div > div > div.b63_b64.b65_b66 > div > button.c3a_c3b.c3f_c3g.c3j_c3k.c3n_c3o.c39_c4a.c4b_c4c").click()
        ...
        // 某日标签的外层按钮不定时改变
        // let dayDom = document.querySelectorAll(".c5z_c50,.c51_c52")
        ...
        setTimeout(() => {
                ...
                for (let j = 0; j < dayArr[0].childNodes[i].childNodes.length; j++) {
                    ...
                    if (day == dayArr[0].childNodes[i].childNodes[j].str) {
                        if (dayArr[0].childNodes[i].childNodes[j].arr[1] == "From") {
                            ...
                            // reqStatus = dayArr[0].childNodes[i].childNodes[j].arr[1]
                            reqStatus = false
                        }
                        break ok
                    }
            ...
            // 日期选择完之后下一步的按钮类名会不定时改变
            // document.querySelector("#main-content > div > div.masthead-carousel-container.masthead-carousel-container--static-content > div.mps-wrapper > div.gutter > div > div > div.calendarsContainer > div > div.epm_epn > footer > div > div > div.cph_cpi.cpj_cpk.cpl_cpm>button").click()
